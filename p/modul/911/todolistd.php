							...
							$no=1;
									...
									$mt = mysqli_query($conn,"select *from tpenjadwalan j left join tassign a on j.kode_tugas=a.NO_PELAPORAN
															  where a.PIC_HANDLING='$_SESSION[username]'
																	...
																	and status_tugas IN ('PLAN','IN PROGRESS','FINISH')
                    ...
                    <div class='card'>
                        <div class='body widget'>
							...
							if($_GET[tek]==NULL){
								...
								$time_menunggusp = mysqli_fetch_array(mysqli_query($conn, "select idProb, statusProblem, dateAction, handling, no from thandling where statusProblem = 'MENUNGGU SPAREPART' AND idProb = '$u[idprob]' AND handling='$u[PIC_HANDLING]'  ORDER BY dateAction desc LIMIT 1"));
								$time_inpro2 = mysqli_fetch_array(mysqli_query($conn, "select idProb, statusProblem, dateAction, handling from thandling where statusProblem = 'IN PROGRESS' AND idProb = '$u[idprob]' and no = '$time_menunggusp[no]'ORDER BY dateAction desc LIMIT 1"));
						...
						$app= mysqli_num_rows(mysqli_query($conn, "select * from mapproval a left join tuserapproval b on a.idApproval=b.idApproval 
							...
							if($app > 0){
								if($g[status_problem] == 'FINISH'){
									echo"<tr><td colspan='2'><br> <a href='$aksi?p=todolist&act=approved&ds=$_GET[ds]&id=$_GET[id]'><button type='button' class='btn btn-danger btn-block'>APPROVED</button></a></td></tr>";
								...
								else if($g[status_problem] == 'APPROVED'){
									echo"<tr><td colspan='2'><br> <a href='$aksi?p=todolist&act=disapproved&ds=$_GET[ds]&id=$_GET[id]'><button type='button' class='btn btn-warning btn-block'>DISAPPROVED</button></a></td></tr>";
								...
								if($u[created_by]==$_SESSION[username]){
									echo "<tr><td colspan='2'><a href='$aksi?p=close&act=close&id=$_GET[id]' title='Klik for closing problem' class='btn btn-sm btn-danger btn-block'>CLOSE</a></td></tr>";
								}
								if($app > 0 && $g[status_problem] == 'APPROVED'){
									echo"<tr><td colspan='2'><br> <a href='$aksi?p=todolist&act=disapproved&ds=$_GET[ds]&id=$_GET[id]'><button type='button' class='btn btn-warning btn-block'>DISAPPROVED</button></a></td></tr>";
									echo"<tr><td colspan='2'><br> <button type='button' class='btn btn-info btn-block' disabled='disabled'>REJECT</button></td></tr>";
								}
							}
							...
							if($app > 0){
								...
								if(mysqli_num_rows($ts)>0){
									...
									if($app > 0){
											...
											}else if($g[status_problem] == 'IN PROGRESS' || $g[status_problem] == 'REJECTED'){
												echo"<tr><td colspan='2'><br /><a href='#tambah_waktu' title='Klik for pending' data-toggle='modal' data-id='$_GET[id]' class='btn btn-block btn-success'>UBAH WAKTU</a></td></tr>";
															...
															<div class='modal-content'>
																...
																<div class='col-sm-12'>
																	...
																	<div class='form-group row m-b-15'>
																		...
																		<div class='col-md-8 col-sm-8'>
																			<input type='text' name='teknisi' class='form-control' value='$tw[fullname]' readonly='readonly'>
																		</div>
																	</div>
																	<div class='form-group row m-b-15'>
																		<label class='col-md-4 col-sm-4 col-form-label' for='website'>EST Handling*</label>
                            ...
                            <div class='body'>
								...
								while($h = mysqli_fetch_array($w)){
									...
									if($h[created_by]==$_SESSION[username]){
										if($_GET[s]=="report"){
											echo"
												<a href='$aksi_tugas?p=todolist&act=deletentctt&id=$_GET[id]&idnote=$h[idjc]&i=$_GET[i]&d=$h[datenote]&t=$h[timenote]&tek=$_GET[tek]' title='Delete' ><i class='icon-trash'></i></a>";
										}else{
											echo"
												<a href='$aksi_tugas?p=todolist&act=deletentctt&id=$_GET[id]&idnote=$h[idjc]&i=$_GET[i]&d=$h[datenote]&t=$h[timenote]&tek=$_GET[tek]' title='Delete' ><i class='icon-trash'></i></a>";
										}
										...
										echo"</td></tr></table>";
										$qq = mysqli_query($conn,"select *from tpenjadwalancatatan where kode_tugas = '$_GET[id]' and datenote = '$h[datenote]' and timenote='$h[timenote]' order by idjc asc");
										$qql = mysqli_query($conn,"select *from tlampirannote where idprob = '$_GET[id]' and dtnote = '$h[datenote]' and tmnote='$h[timenote]' order by id_lampirannote asc");
                ...
                <div class='col-lg-4 col-md-12 right-box'>
                    <div class='card'>
                        <div class='body widget'>
							...
							<tr><td>Approved Pada </td><td>: $time_app[dateAction]</td></tr>
							<tr><td>Status </td><td>: <b>$u[status_tugas]</b></td></tr>
							<tr><td>Real Waktu Kerja </td><td>:</td></tr>
							<tr><td colspan='2' align='center'><b>$real_day Hari, $real_hour Jam, $real_min Menit</b></td></tr>
							</table>
							<br />";
	...
	<div class='modal-dialog-large'>
		<div class='modal-content'>
			...
			<div class='form-group row m-b-15'>
				...
				<div class='form-group row m-b-15'>
					<label class='col-md-4 col-sm-4 col-form-label' for='website'>Deskripsi*</label>
					<div class='col-md-8 col-sm-8'>
