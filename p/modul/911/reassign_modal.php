...
if($_POST['idx']) {
        $id = $_POST['idx'];      
        $sql = mysql_query("SELECT * FROM tassign a join tlaporan b on a.no_pelaporan=b.no_pelaporan WHERE a.no_pelaporan = '$id'");
        while ($r = mysql_fetch_array($sql)){
		echo "
        <form class='row clearfix' method='POST' action='$aksi?p=assign&act=reassign'>
			<input type='hidden' name='idprob' value='$r[NO_PELAPORAN]'>
            <div class='col-sm-12'>
				<div class='form-group row m-b-15'>
					<label class='col-md-4 col-sm-4 col-form-label' for='website'>Re-Assign To*</label>
					<div class='col-md-8 col-sm-8'>
						<select class='default-select2 form-control' name='assign_group' required>
							<option value=''>--Select EDP Personil--</option>";
							$q = mysql_query("select FULLNAME,USERNAME from user where divisi='EDP' and active=1 order by fullname asc");
							while($p = mysql_fetch_array($q)){
								if($p[USERNAME]==$r[PIC_HANDLING]){
					...
					<div class='col-md-8 col-sm-8'>
					  <div class='form-group'>
						<select class='default-select2 form-control' name='problem_group' required onchange='test()' id='problem_group'>
							...
							while($p = mysql_fetch_array($q)){
								if($p[ID_JENIS_PROBLEM]==$r[JENIS_PROBLEM]){
