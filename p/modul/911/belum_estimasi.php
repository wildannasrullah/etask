...
<div class='block-header'>
                <div class='row'>
                    <div class='col-lg-5 col-md-8 col-sm-12'>                        
                        <h2><a href='javascript:void(0);' class='btn btn-xs btn-link btn-toggle-fullwidth'><i class='fa fa-arrow-left'></i></a> Belum Input Estimasi Waktu</h2>
                        <ul class='breadcrumb'>
                            <li class='breadcrumb-item'><a href='?p=dashboard'><i class='icon-home'></i></a></li>                            
                            <li class='breadcrumb-item'><font color='red'>Daftar Problem yang di "Assign To" otomatis oleh sistem</font></li>
                        </ul>
                    </div>         
                </div>
            </div>
						...
						echo "
                            ...
                            <div class='table-responsive'>
								...
								$date = date('Y-m-d');
									...
									$ro = mysqli_query($conn,"SELECT t.NO_PELAPORAN, t.PIC_HANDLING, t.CREATED_BY as assginby, t.EST_DAY, t.EST_HOUR, t.EST_MIN, a.*, m.* 
																from tassign t left join tproblems a on a.idprob=t.NO_PELAPORAN
																left join mcategories m on a.idcat = m.idcat
																where EST_DAY IS NULL OR EST_HOUR IS NULL OR EST_MIN IS NULL order by a.idcat asc;");
								
									while($r = mysqli_fetch_array($ro)){
										
									$l = mysqli_fetch_array(mysqli_query($conn,"select *from user where username = '$r[PIC_HANDLING]'"));
									...
									echo "
											...
											<td><a href='?p=todolist&act=problem-detail&id=$r[idprob]'><font color='$color'><b>$r[idprob]</b></font></a></td>
                                            <td><font color='$color'>$l[fullname]</font></td>
											<td><font color='$color'>$r[divisi_problem]</font></td>
                                            <td><font color='$color'>$r[created_at]</font></td>
											<td><font color='$color'>$r[status_problem]</font></td>
											<td><font color='$color'>$r[assginby]</font></td>
											<td><a href='?p=todolist&act=problem-detail&id=$r[idprob]&s=input_estim'><button type='button' class='btn btn-success btn-sm' >INPUT WAKTU</button></td>
											</tr>";
										$no++;
									}
